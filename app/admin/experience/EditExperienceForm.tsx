...
export default function EditExperienceForm({ experience, onClose, onUpdated }: { experience: any, onClose: () => void, onUpdated: (exp: any) => void }) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            const res = await axios.put(`/api/admin/experience/${experience.id}`, payload);
            if (res.data.success) {
                onUpdated(res.data.experience);
                onClose();
            }
        } catch (error) {
            console.error(error);
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input className="w-full px-4 py-2 rounded-lg border bg-background" value={position} onChange={e => setPosition(e.target.value)} />
                        </div>
                        <div>
                            <label className="text-sm font-medium">Company</label>
                            <input className="w-full px-4 py-2 rounded-lg border bg-background" value={company} onChange={e => setCompany(e.target.value)} />
                        </div>
                    </div>

                    <div>
                        <label className="text-sm font-medium">Period</label>
                        <input className="w-full px-4 py-2 rounded-lg border bg-background" value={period} onChange={e => setPeriod(e.target.value)} />
                    </div>

                    <div>
                        <label className="text-sm font-medium">Description</label>
                        <textarea className="w-full px-4 py-2 rounded-lg border bg-background h-20 resize-none" value={description} onChange={e => setDescription(e.target.value)} />
                    </div>
